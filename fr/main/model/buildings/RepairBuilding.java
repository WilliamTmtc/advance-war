...
public interface RepairBuilding extends AbstractBuilding {
     ...
     * @return true if and only if the building can repair the unit given in
     *     parameter
     */
    ...
    public default void repair(AbstractUnit u) {
        if (canRepair(u)) {
            if (this instanceof OwnableBuilding) {
                ...
                // the building cannot add life such as the unit has more than
                // 100 life points one life point costs 1/80 of the unit's cost
                // so the final formula is the one below :
                int pv = Math.max(0, Math.min(Math.min(100 - u.getLife(), 20),
                                              80 * p.getFunds() / u.getCost()));
                u.addLife(pv);
