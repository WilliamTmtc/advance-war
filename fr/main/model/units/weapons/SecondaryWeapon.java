...
public class SecondaryWeapon extends Weapon {
    ...
    public SecondaryWeapon(
        String name, Map<Class<? extends AbstractUnit>, Integer> damages) {
    ...
    public boolean isInRange(AbstractUnit shooter, AbstractUnit target) {
        return Math.abs(shooter.getX() - target.getX()) +
            Math.abs(shooter.getY() - target.getY()) ==
            1;
    }
    ...
    public void renderTarget(boolean[][] map, AbstractUnit u, int x, int y) {
        if (u.getMoveQuantity() != 0)
            for (Direction d : Direction.cardinalDirections())
                if (y + d.x >= 0 && y + d.x < map.length && x + d.y >= 0 &&
                    x + d.y < map.length)
    ...
    public boolean canAttack(AbstractUnit shooter, AbstractUnit target) {
        return target != null && shooter.getPlayer() != target.getPlayer() &&
            shooter.isEnabled() && canShoot(target) &&
            isInRange(shooter, target);
    }
}