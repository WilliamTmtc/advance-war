...
public class Minimap extends InterfaceUI {
    ...
    public void draw(Graphics g) {
        ...
        int x = camera.getX(),
            y = camera.getY(), w = world.getDimension().width,
            h = world.getDimension().height,
            mx = reference.leftSide
                     ? reference.x + TerrainPanel.WIDTH + TerrainPanel.MARGIN
                     : reference.x - TerrainPanel.MARGIN - w * size,
            my = reference.y;

        // TODO: more efficient stuff
        for (int i = 0; i < w; i++)
            for (int j = 0; j < w; j++) {
                int a = mx + j * size, b = my + i * size;
                AbstractUnit unit = world.getUnit(j, i);
                if (unit != null &&
                    (unit.getPlayer() == world.getCurrentPlayer() ||
                     world.isVisibleOpponentUnit(j, i))) {
                    ...
                    if (building != null) {
                        ...
                        if (t instanceof LandTerrain)
                            g.setColor(Color.green);
                        else if (t instanceof NavalTerrain)
