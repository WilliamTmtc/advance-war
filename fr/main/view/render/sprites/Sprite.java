...
public class Sprite {
    ...
    public Image getImage(int x, int y, int w, int h, double scale) {
        ...
        return img.getScaledInstance((int)(((double)w) * scale),
                                     (int)(((double)h) * scale),
                                     Image.SCALE_SMOOTH);
    }

    public Image getReverseImage(int x, int y, int w, int h, double scale,
                                 boolean vertical) {
        ...
        return op.filter(img, null).getScaledInstance(
            (int)(((double)w) * scale), (int)(((double)h) * scale),
            Image.SCALE_SMOOTH);
    }
    ...
    public Image getImage(ScaleRect rect) {
        ...
        if (rect.reverse == ScaleRect.Flip.HORIZONTALLY)
            return getReverseImage(rect.x, rect.y, rect.width, rect.height,
                                   rect.scale, true);
    ...
    public boolean contains(String path) { return instances.containsKey(path); }
    ...
    public class SpriteTuple {
        ...
        public ScaleRect getRect() { return rect; }
    }
}
