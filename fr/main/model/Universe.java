...
public class Universe {
    ...
    public static class Board implements Serializable {
        ...
        public Board(AbstractUnit[][] units, Player[] ps,
                     AbstractTerrain[][] board, AbstractBuilding[][] buildings,
                     Weather weather, Player current, int day) {
            this.fog = true;
            this.board = board;
            this.units = units;
            this.players = ps;
        ...
        public Board setPlayers(Player[] ps) {
            ...
            for (int i = 0; i < this.players.length; i++) {
                ...
                if (hq == null)
                    System.out.println("No HQ for " + tmp.name + "from " +
                                       players[i].name);
                else {
                    hq.changeOwner(tmp);
                    if (tmp == null)
                        board.buildings[hq.getY()][hq.getX()] =
                            new City(null, new Point(hq.getX(), hq.getY()));
                }
            }
    ...
    /**
     * An Iterator<Player> which is a cycle (when we're on the last player, the
     * next is the first)
     */
    protected final Cycle players;
    /**
     * The vision of the map (the tiles that the current player can see are
     * true)
     */
    ...
    public Universe(AbstractUnit[][] units, AbstractTerrain[][] map,
    ...
    public static Board restaure(String mapName) {
        if (save)
            try (FileInputStream fileIn =
                     new FileInputStream(mapPath + mapName);
     ...
     * When a player looses, if there is only one player left then the game is
     * done
     */
    ...
    public Dimension getDimension() {
        if (size == null)
            size = new Dimension(map.board[0].length, map.board.length);
        return size;
    }
     ...
     */
    public Player getCurrentPlayer() { return map.current; }

    /**
     * @return true if the point given in parameter can be seen by the current
     *     player
     */
     ...
     *     player
     */
    public boolean isVisible(Point pt) { return isVisible(pt.x, pt.y); }
     ...
     */
    public int getDay() { return map.day; }
     ...
     */
    public void bigPower() { getCurrentPlayer().getCommander().activate(true); }
     ...
     * @return the unit at the specified location
     */
    public final AbstractUnit getUnit(Point pt) { return getUnit(pt.x, pt.y); }

    /**
     * @return true if and only if the specified location is valid
     */
    public final boolean isValidPosition(int x, int y) {
        return y >= 0 && x >= 0 && y < map.units.length &&
            x < map.units[0].length;
    }
    ...
    /**
     * @param x is the horizontal coordinate of the tile in which we want to
     *     place the unit
     * @param y is the vertical coordinate of the tile in which we want to place
     *     the unit
    ...
    public boolean setBuilding(int x, int y, AbstractBuilding b) {
        if (isValidPosition(x, y)) {
            ...
            // the AI reads the whole map at the beginning to search for
            // buildings that will be considered as objectives, if a building is
            // changed, the AI is warned
            ...
            for (int j = 0; j < bss[0].length; j++)
                if (!(bss[i][j] instanceof GenericBuilding))
                    if (!setBuilding(i, j, bss[i][j]))
                        System.err.println(bss[i][j].toString() +
                                           " not initialized in Universe.");
    }
     ...
     * @return true if and only if there is an opponent unit at the location
     *     specified and the unit is visible by the current player
     */
    public boolean isVisibleOpponentUnit(int x, int y) {
        AbstractUnit unit = getUnit(x, y);
        if (isVisible(x, y) && unit != null &&
            unit.getPlayer() != getCurrentPlayer()) {
            if (unit instanceof HideableUnit && ((HideableUnit)unit).hidden()) {
                for (Direction d : Direction.cardinalDirections())
                    if (getUnit(x + d.x, y + d.y) != null &&
                        getUnit(x + d.x, y + d.y).getPlayer() ==
                            getCurrentPlayer())
                        return true;
                return false;
            }
     ...
     */
    public static void save(String mapName, AbstractUnit[][] units,
                            AbstractTerrain[][] map, Player[] ps,
                            AbstractBuilding[][] buildings) {
    ...
    public static void save(String mapName, Board board) {
        ...
        try {
            ...
            out.close();
            fileOut.close();
            System.out.println("Serialized data is saved in " + mapPath +
                               mapName);
        } catch (IOException i) {
            i.printStackTrace();
        }
    ...
    public void save(String mapName) { Universe.save(mapName + ".map", map); }

    /**
     * Saves the game with a default name with the format
     * year-month-day-hour-minute-second
     */
    public void save() {
        Universe.save((new SimpleDateFormat("yyyy-MM-dd-HH-mm-ss"))
     ...
     */
    public static Universe get() { return instance; }

    /**
     * @return the weather
     */
    public Weather getWeather() { return map.weather; }

    /**
     * @return the height of the map
     */
    public int getMapHeight() { return map.board.length; }

    /**
     * @return the width of the map
     */
    public int getMapWidth() { return map.board[0].length; }

    public int getNumberOfPlayers() { return this.map.players.length; }
}
