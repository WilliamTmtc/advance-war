...
public class PlayerPanel extends InterfaceUI {
    ...
    protected void draw(Graphics g) {
        int halfw = MainFrame.width() / (2 * MainFrame.UNIT),
            ...
            x = cursor.getX() - camera.getX() >= halfw &&
                        cursor.getY() - camera.getY() <= halfh
                    ? MARGIN
                    : MainFrame.width() - WIDTH - MARGIN,
            y = MARGIN;
        ...
        g.fillRect(x, y + HEIGHT - 20,
                   c.powerBar.getValue() * WIDTH / c.powerBar.maxValue, 20);
        g.setColor(Color.black);

        if (c.getPowerCost(false) <= c.powerBar.getValue())
            g.setColor(Color.blue);
        int r = x + c.getPowerCost(false) * WIDTH / c.powerBar.maxValue;
        g.drawLine(r, y + HEIGHT - 20, r, y + HEIGHT);

        if (c.getPowerCost(true) > c.powerBar.getValue())
            g.setColor(Color.black);
