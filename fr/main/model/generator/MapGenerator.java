...
public class MapGenerator {
    ...
    private int seaProportion, landProportion, mountainProportion,
        woodProportion, smoothness, power, seaBandSize, mapHeight, mapWidth;

    /**
     * Generates a MapGenerator of seed 1 and for 2 players.
     */
    public MapGenerator() { this(1, 2); }
     ...
     * @param seed
     */
    public MapGenerator(int seed, int playerNb) { this((long)seed, playerNb); }
    ...
    public Player[] getLastPlayers() { return lastPlayers; }

    public TerrainEnum[][] getLastMap() { return lastMap; }
    ...
    public int getLandProportion() { return landProportion; }

    public int getSeaProportion() { return seaProportion; }
    ...
    public int getSeaBandSize() { return seaBandSize; }

    public int getMapHeight() { return mapHeight; }

    public int getMapWidth() { return mapWidth; }

    public int getMountainProportion() { return mountainProportion; }

    public int getWoodProportion() { return woodProportion; }
    ...
    public int[][] getLastHQCoordinates() { return lastHQCoordinates; }

    public boolean isStarterAirport() { return starterAirport; }

    public boolean isStarterBarrack() { return starterBarrack; }

    public AbstractBuilding[][] getCurrentBuildingLayout() {
        return currentBuildingLayout;
    }

    public TerrainEnum[][] getCurrentMap() { return currentMap; }
    ...
    public boolean isStarterDock() { return starterDock; }
    ...
    public void setSeaBandSize(int seaBandSize) {
        if (seaBandSize < 0)
            seaBandSize = 0;
        //		else if (seaBandSize > mapHeight / 2) seaBandSize =
        //mapHeight / 2 + 1; 		else if (seaBandSize > mapWidth / 2) seaBandSize =
        //mapWidth / 2 + 1;
    ...
    public void setPower(int p) { this.power = makeInBound(0, 10, p); }

    public void setLandProportion(int landProportion) {
        this.landProportion = makeInBound(
            0, 100 - (seaProportion - (seaProportion > 20 ? 20 : 0)),
            landProportion);
    }

    public void setSeaProportion(int seaProportion) {
        this.seaProportion = makeInBound(
            0, 100 - (landProportion - (landProportion > 20 ? 20 : 0)),
            seaProportion);
    }
    ...
    public void setPlaceSilo(boolean placeSilo) { this.placeSilo = placeSilo; }

    public TerrainEnum[][] randMap() { return randMap(mapHeight, mapWidth); }
    ...
    public TerrainEnum[][] randMap(int x, int y) {
        ...
        for (TerrainEnum[] line : currentMap)
            Arrays.fill(line, none);
        ...
        currentBuildingLayout =
            placeBuildings(currentBuildingLayout, currentMap);
        ...
        for (int i = 0; i < x; i++) {
            for (int j = 0; j < y; j++) {
                if (currentMap[i][j] == none) {
                    currentMap[i][j] = getTerrainEnum(
                        referencePoints[closestPoint(i, j, referencePoints)]
                                       [2]);
                }
            }
        }
        ...
        currentBuildingLayout =
            placeStarterDocks(currentMap, currentBuildingLayout);
     ...
     */
    public void resetCurrentHQCorrdinates() { currentHQCoordinates = null; }
    ...
    /**
     * @param layout
     * @return building layout with placed starter building except the docks
     *     (seperate function)
     */
    private AbstractBuilding[][] placeStarterBuildings(
        AbstractBuilding[][] layout) {
        int[][] hqs = getCurrentHQCoordinates();
        ...
        for (int[] coor : hqs) {
            sqrCoor =
                getSquare(layout.length, layout[0].length, coor[0], coor[1]);
            if (sqrCoor.length == 0) {
            ...
            if (starterBarrack) {
                while (true) {
                    ...
                    if (!(layout[sqrCoor[randNb][0]]
                                [sqrCoor[randNb][1]] instanceof
                          GenericBuilding))
                        continue;
     ...
     */
    private AbstractBuilding[][] placeStarterDocks(
        TerrainEnum[][] map, AbstractBuilding[][] layout) {
        if (!starterDock)
            return layout;
        ...
        for (int[] coor : hqs) {
            ...
            int[][] nearest =
                findNearestTerrainEnums(map, coor[0], coor[1], sea);
            for (int i = 0; i < 30; i++) {
                ...
                if (starterDock)
                    layout[nearest[randNb][0]][nearest[randNb][1]] = new Dock(
                        ((Headquarter)layout[coor[0]][coor[1]]).getOwner(),
                        new Point(nearest[randNb][1], nearest[randNb][0]));
                else
                    layout[nearest[randNb][0]][nearest[randNb][1]] =
                        new Dock(null, new Point(nearest[randNb][1],
                                                 nearest[randNb][0]));
                break;
            }
        }
     ...
     */
    private AbstractBuilding[][] placeHeadQuarters(int mapHeight,
                                                   int mapWidth) {
     ...
     * @return building layout with placed HQs.
     */
    private AbstractBuilding[][] placeHeadQuarters(
        AbstractBuilding[][] layout) {
        int x = layout.length, y = layout[0].length;
        ...
        //		AbstractBuilding[] rect = getRectangleFromSide(layout,
        //seaBandSize + 1 + randNb);

        AbstractBuilding[] rect =
            getRectangle(layout, x / 2, y / 2, x / 2 - seaBandSize - randNb - 2,
                         y / 2 - seaBandSize - randNb - 2);
        ...
        case 2:
            hqs.add(useWidth ? rand.nextInt() % realY
                             : rand.nextInt() % realX + realY - 1);
        ...
        case 4: {
            ...
            for (int i = 3; i >= 0; i--) {
                if (i > 0)
                    pop = abs(rand.nextInt(i));
                else
                    pop = 0;
                hqs.add(l.get(pop));
                if (l.size() <= pop)
                    hqs.remove(hqs.size() - 1);
                else
                    l.remove(pop);
            }
            size = 4;
        ...
        for (int i = 0; i < hqs.size(); i++) {
            layout[rect[hqs.get(i)].getX()][rect[hqs.get(i)].getY()] =
                new Headquarter(players[i], new Point(rect[hqs.get(i)].getY(),
                                                      rect[hqs.get(i)].getX()));
     ...
     * @return an array of Abstract building on the tiles distant of
     *     distanceFromSide for each side.
     */
    private AbstractBuilding[] getRectangleFromSide(AbstractBuilding[][] layout,
     ...
     * @param x0
     * @param y0
     * @return array of AbstractBuildings on the tiles on the side of a square
     *     of size
     * 3 and of center (x0, y0).
     */
    private AbstractBuilding[] getSquare(AbstractBuilding[][] layout, int x0,
                                         int y0) {
     ...
     * @param distH
     * @param distW
     * @return array of AbstractBuildings on the tiles on the side of a
     *     rectangle of size
     * (distH * 2 + 1)x(distW * 2 + 1) and of center (x0, y0).
     */
    private AbstractBuilding[] getRectangle(AbstractBuilding[][] layout, int x0,
                                            int y0, int distH, int distW) {
        if (layout == null)
            return new AbstractBuilding[0];
        return coordinatesToAbstractBuilding(
            getRectangle(layout.length, layout[0].length, x0, y0, distH, distW),
            layout);
    }
     ...
     */
    private int[][] getRectangle(int height, int width, int x0, int y0,
                                 int distH, int distW) {
        int top = x0 - distH, bottom = x0 + distH, left = y0 - distW,
            right = y0 + distW;
     ...
     */
    private int[][] getDiamond(int height, int width, int x0, int y0,
                               int distFromCenter) {
        if (distFromCenter < 0)
        ...
        for (int i = 0; i < last.length; i++) {
            item = new int[2];
            if (last[i][0] <= x0 &&
                isInRect(last[i][0] - 1, last[i][1], height, width)) {
                item = new int[2];
                item[0] = last[i][0] - 1;
                item[1] = last[i][1];
                coors.add(item);
            ...
            if (last[i][0] == x0 && last[i][1] >= y0 &&
                isInRect(x0, last[i][1] + 1, height, width)) {
                item = new int[2];
                item[0] = x0;
                item[1] = last[i][1] + 1;
                coors.add(item);
     ...
     */
    private AbstractBuilding[] coordinatesToAbstractBuilding(
        int[][] coors, AbstractBuilding[][] layout) {
        int count = 0;
     ...
     */
    private int[][] findNearestTerrainEnums(TerrainEnum[][] map, int x, int y,
                                            TerrainEnum type) {
     ...
     * @param x
     * @param y
     * @return the coordinates of the tiles adjacent (in a cross) to tile (x,
     *     y).
     */
     ...
     * @param map of tiles.
     * @return map with setted mLocations that will remain unchanged until the
     *     end of
     * the generation of the map.
     */
    private TerrainEnum[][] setImmuableTerrain(AbstractBuilding[][] layout,
                                               TerrainEnum[][] map) {
        for (int i = 0; i < layout.length; i++)
            for (int j = 0; j < layout[0].length; j++) {
                if (layout[i][j] instanceof Headquarter)
    ...
    private TerrainEnum[][] fillSquareFromCenter(TerrainEnum[][] map, int x,
                                                 int y, TerrainEnum filler,
                                                 int distanceFromCenter) {
        if (isInMap(map, x, y) && distanceFromCenter > 0)
            for (int i = x - distanceFromCenter; i <= x + distanceFromCenter;
                 i++)
                for (int j = y - distanceFromCenter;
                     j <= x + distanceFromCenter; j++)
    ...
    private TerrainEnum[][] cellAutomaton(TerrainEnum[][] map) {
        ...
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[0].length; j++) {
                count = 0;
                if (i - 1 >= 0 && map[i - 1][j] == lowland)
                    count++;
                if (i + 1 < map.length && map[i + 1][j] == lowland)
                    count++;
                if (j - 1 >= 0 && map[i][j - 1] == lowland)
                    count++;
                if (j + 1 < map[0].length && map[i][j + 1] == lowland)
                    count++;
                switch (count) {
                case 0:
                    mapBis[i][j] = sea;
        ...
        for (int i = 0; i < map.length; i++) {
            for (int j = 0; j < map[0].length; j++) {
                if (map[i][j] == sea)
                    continue;
                count = 0;
                if (i - 1 >= 0 && map[i - 1][j] == sea)
                    count++;
                if (i + 1 < map.length && map[i + 1][j] == sea)
                    count++;
                if (j - 1 >= 0 && map[i][j - 1] == sea)
                    count++;
                if (j + 1 < map[0].length && map[i][j + 1] == sea)
                    count++;
        ...
        for (int i = 0; i < map.length; i++)
            for (int j = 0; j < map[0].length; j++) {
                if (map[i][j] == beach &&
                    ...
                    map[i][j] = lowland;
                if (getAdjacentTerrainNb(map, i, j, beach) == 1 &&
                    getAdjacentTerrainNb(map, i, j, lowland) == 2)
                    map[i][j] = lowland;
            }
     ...
     */
    private AbstractBuilding[][] placeBuildings(AbstractBuilding[][] layout,
                                                TerrainEnum[][] map) {
        ...
        for (int i = 0; i < cityRingNb; i++) {
            ...
            layout = placeBuildingRing(
                layout, map, bs,
                rand.nextInt(layout.length - 2 * seaBandSize - 2));
        }
        ...
        if (placeSilo) {
            layout[layout.length / 2][layout[0].length / 2] =
                new MissileLauncher(
                    new Point(layout[0].length / 2, layout.length / 2));
        }
     ...
     */
    private AbstractBuilding[][] placeBuildingRing(
        AbstractBuilding[][] layout, TerrainEnum[][] map,
        LinkedList<AbstractBuilding> buildings, int distFromCenter) {
        if (buildings.size() == 0) {
        ...
        int distH =
            (int)(useH ? (float)distFromCenter * proportion : distFromCenter);
        int distW =
            (int)(useH ? distFromCenter : (float)distFromCenter * proportion);
        ...
        for (int i = 0; i < map.length; i++)
            for (int j = 0; j < map[0].length; j++) {
                if (map[i][j] == lowland &&
                    ...
                    if (mountains == 0 &&
                        rand.nextInt(
                            20 - (int)((float)mountainProportion / 5.55)) < 1)
                        map[i][j] = mountain;
                    else if (mountains > 0 && rand.nextInt(15) < 4)
                        map[i][j] = mountain;
                    else if (mountains == 0 && rand.nextInt(12) < 2)
                        map[i][j] = hill;
                    else if ((mountains > 0 ||
                              getAdjacentTerrainNb(map, i, j, hill) > 0) &&
                             rand.nextInt(12) < 3)
                        map[i][j] = hill;
                }
            }
            ...
            for (int j = 0; j < map[0].length; j++) {
                if (map[i][j] == lowland || map[i][j] == hill)
                    if (getAdjacentTerrainNb(map, i, j, wood) == 0)
                        map[i][j] =
                            (rand.nextInt(
                                 20 - (int)((float)woodProportion / 5.55)) < 2)
                                ? wood
                                : map[i][j];
                    else
                        map[i][j] =
                            (rand.nextInt(
                                 10 - (int)((float)woodProportion / 11.1)) < 2)
                                ? wood
                                : map[i][j];
            }
        return map;
     ...
     * @return layout with last modifications left from the different gen
     *     algorithms.
     */
    ...
    /**
     * @param map to clean.
     * @return map with last modifications left from the different gen
     *     algorithms.
     */
    private TerrainEnum[][] clean(TerrainEnum[][] map) {
        ...
        for (int i = 0; i < map.length; i++)
            for (int j = 0; j < map[0].length; j++) {
                ...
                case beach:
                    if (getAdjacentTerrainNb(map, i, j, sea) >= 3 &&
                        getAdjacentTerrainNb(map, i, j, land) == 0)
                        map[i][j] = sea;
                ...
                case sea:
                    ...
                    break;
                case river:
                    if (getAdjacentTerrainNb(map, i, j, sea) +
                            getAdjacentTerrainNb(map, i, j, river) ==
                        0)
                        map[i][j] = wood;
                    break;
                case road:
                    ...
                    int[][] sqr =
                        getFilledSquare(map.length, map[0].length, i, j, 2);
                    ...
                    if (!clean) {
                        ...
                        for (int k = 0; k < sqr.length; k++) {
                            if (getAdjacentTerrainNb(map, sqr[k][0], sqr[k][1],
                                                     road) +
                                    getAdjacentTerrainNb(map, sqr[k][0],
                                                         sqr[k][1], bridge) <
                                3)
                                remove.add(k);
                        }
    ...
    private TerrainEnum[][] cleanLowland(TerrainEnum[][] map, int x, int y) {
        ...
        if (isInMap(map, x + 2, y) && map[x + 2][y] == sea &&
            isInMap(map, x + 1, y) && map[x + 1][y] == sea) {
            if (isInMap(map, x, y + 1) && map[x][y + 1] == sea &&
                isInMap(map, x + 2, y + 1) &&
                hasMatch(land, map[x + 2][y + 1]) &&
     ...
     */
    private int[][] getFilledSquare(int mapHeight, int mapWidth, int x0, int y0,
                                    int size) {
    ...
    private TerrainEnum[][] placeRivers(TerrainEnum[][] map) {
        TerrainEnum[] land = {lowland, bridge, mountain, hill, wood},
                      naval = {none, sea, river};
                ...
                if (map[i][j] == sea) {
                    if (isSandwiched(map, i, j, land) &&
                        getAdjacentTerrainNb(map, i, j, lowland) +
                                getAdjacentTerrainNb(map, i, j, beach) <
                            3)
                        map[i][j] = river; // continue;
                    else if (getAdjacentTerrainNb(map, i, j, sea) +
                                     getAdjacentTerrainNb(map, i, j, river) <=
                                 2 &&
                             isSandwiched(map, i, j, naval))
                        map[i][j] = river;
        ...
        for (int i = 0; i < map.length; i++)
            for (int j = 0; j < map[0].length; j++)
                if (map[i][j] == sea &&
                    getAdjacentTerrainNb(map, i, j, river) >= 2)
                    map[i][j] = river;
        return map;
    }
     ...
     */
    public static TerrainEnum[] getSurroundingTerrain(TerrainEnum[][] map,
                                                      int x, int y) {
        TerrainEnum[] out = new TerrainEnum[8];
        Arrays.fill(out, none);

        if (isInMap(map, x - 1, y))
            out[0] = map[x - 1][y];
        if (isInMap(map, x - 1, y + 1))
            out[1] = map[x - 1][y + 1];
        if (isInMap(map, x, y + 1))
            out[2] = map[x][y + 1];
        if (isInMap(map, x + 1, y + 1))
            out[3] = map[x + 1][y + 1];
        if (isInMap(map, x + 1, y))
    ...
    public static boolean isSandwiched(TerrainEnum map[][], int x, int y,
                                       TerrainEnum type) {
        return isSandwiched(map, x, y, type, true) ||
            isSandwiched(map, x, y, type, false);
    }
    ...
    public static boolean isSandwiched(TerrainEnum map[][], int x, int y,
                                       TerrainEnum type, boolean horizontal) {
        TerrainEnum[] ts = {type};
    ...
    public static boolean isSandwiched(TerrainEnum map[][], int x, int y,
                                       TerrainEnum type[], boolean horizontal) {
        return (!horizontal && isInMap(map, x - 1, y) &&
                isInMap(map, x + 1, y) && hasMatch(type, map[x - 1][y]) &&
                hasMatch(type, map[x + 1][y])) ||
            (horizontal && isInMap(map, x, y - 1) && isInMap(map, x, y + 1) &&
             hasMatch(type, map[x][y - 1]) && hasMatch(type, map[x][y + 1]));
    }
     ...
     * @param y
     * @param type
     * @return the number of cells in the cardinal orientation that are of type
     *     "type".
     */
    private int getAdjacentTerrainNb(TerrainEnum[][] map, int x, int y,
                                     TerrainEnum type) {
        int count = 0;

        if (isInMap(map, x + 1, y) && map[x + 1][y] == type)
            count++;
        if (isInMap(map, x - 1, y) && map[x - 1][y] == type)
            count++;
        if (isInMap(map, x, y + 1) && map[x][y + 1] == type)
            count++;
        if (isInMap(map, x, y - 1) && map[x][y - 1] == type)
            count++;
    ...
    private TerrainEnum[][] surroundBySea(TerrainEnum[][] map, int size) {
        for (int i = 0; i < map.length; i++)
            for (int j = 0; j < map[0].length; j++)
                if (i <= size || map.length - i - 1 <= size || j <= size ||
                    map[0].length - j - 1 <= size)
                    map[i][j] = sea;
        return map;
    }

    private int[][] placeRoadNodes(TerrainEnum[][] map,
                                   AbstractBuilding[][] layout) {
        AbstractBuilding b;
        int randNb;
        int[][] cross;
                ...
                if (isNonGenericBuilding(b)) {
                    if (b instanceof Headquarter || rand.nextInt(10) >= 2) {
                        ...
                        for (int k = 0; k < 30; k++) {
                            ...
                            if (isNonGenericBuilding(
                                    layout[cross[randNb][0]]
                                          [cross[randNb][1]]) ||
                                map[cross[randNb][0]][cross[randNb][1]] ==
                                    sea ||
     ...
     */
    private TerrainEnum[][] placeRoads(TerrainEnum[][] map,
                                       AbstractBuilding[][] layout,
                                       int[][] roadNodes) {
        ...
        for (int[] coor : roadNodes) {
            //			int p = closestPoint(coor[0], coor[1],
            //roadNodes);
            aStar.setWeights(getWeights(map));
            randNb = rand.nextInt(roadNodes.length);
            road = aStar.shortestRoute(coor[0], coor[1], roadNodes[randNb][0],
                                       roadNodes[randNb][1]);
    ...
    public static boolean isInRect(int x, int y, int x0, int y0, int height,
                                   int width) {
    ...
    private static boolean isInLayout(AbstractBuilding[][] layout, int x,
                                      int y) {
        return layout != null &&
            isInRect(x, y, layout.length, layout[0].length);
    }
    ...
    /**
     * @param map
     * @return an array of array of weights for the pathfinding algo for the
     *     roads placement.
     */
    ...
    /**
     * @param layout
     * @return an array of array of boolean reprensenting if a can be a road or
     *     not
    ...
    private boolean[][] getBlocks(AbstractBuilding[][] layout) {
        ...
        for (int i = 0; i < layout.length; i++)
            for (int j = 0; j < layout[0].length; j++)
                if (layout[i][j] != null &&
                    !(layout[i][j] instanceof GenericBuilding))
                    blocks[i][j] = true;
        return blocks;
    }
}
