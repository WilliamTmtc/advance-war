...
public class MapRenderer extends Universe {
    ...
    protected class Frame {
        ...
        public void forEach(BiConsumer<Integer, Integer> func) {
            for (int i = firstY; i < Math.min(lastY, map.board.length); i++)
                for (int j = firstX; j < Math.min(lastX, map.board[i].length);
                     j++)
                    func.accept(i, j);
        }
    }
    ...
    public MapRenderer(AbstractUnit[][] units, AbstractTerrain[][] map,
                       Player[] ps, AbstractBuilding[][] buildings) {
        this(new Universe.Board(units, ps, map, buildings));
    }

    public MapRenderer(String mapName) { this(Universe.restaure(mapName)); }
    ...
    public Frame draw(Graphics g, int x, int y, int offsetX, int offsetY) {
        g.setFont(font);
        int w = map.board.length, h = map.board[0].length;
