...
public class Cruiser extends Unit implements NavalUnit, TransportUnit {
    ...
    private static final Map<Class<? extends AbstractUnit>, Integer>
        PRIMARYWEAPON_DAMAGES =
            new HashMap<Class<? extends AbstractUnit>, Integer>();
    private static final Map<Class<? extends AbstractUnit>, Integer>
        SECONDARYWEAPON_DAMAGES =
            new HashMap<Class<? extends AbstractUnit>, Integer>();
    ...
    static {
        ...
        SECONDARYWEAPON =
            new SecondaryWeapon(SECONDARYWEAPON_NAME, SECONDARYWEAPON_DAMAGES);
    }

    public Cruiser(Player player, Point point) {
        super(player, point, fuelName, 99, true, MoveType.NAVAL, 6, 3,
              new PrimaryWeapon(PRIMARYWEAPON_NAME, 9, PRIMARYWEAPON_DAMAGES,
                                false),
              SECONDARYWEAPON, NAME, PRICE);
    }
    ...
    public boolean isFull() { return getCapacity() < units.size(); }
    ...
    public boolean canCharge(AbstractUnit u) {
        return !isFull() && u.getPlayer() == getPlayer() &&
            !units.contains(u) && u.moveCost(getX(), getY()) != null &&
            u instanceof CopterUnit;
    }
    ...
    public boolean canRemove(AbstractUnit u, int x, int y) {
        return units.contains(u) &&
            Math.abs(x - getX()) + Math.abs(y - getY()) == 1 &&
            u.moveCost(getX(), getY()) != null && u.canStop(x, y);
    }
    ...
    public int getFuelTurnCost() { return 1; }
}