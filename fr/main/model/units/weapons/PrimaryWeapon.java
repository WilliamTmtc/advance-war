...
/**
 * Primary weapons are weapons with limited ammunitions. They may be range
 * weapons.
 */
public class PrimaryWeapon extends Weapon {
    ...
    public PrimaryWeapon(String name, int maximumAmmo, int minimumRange,
                         int maximumRange,
                         Map<Class<? extends AbstractUnit>, Integer> damages,
    ...
    public PrimaryWeapon(String name, int maximumAmmo,
                         Map<Class<? extends AbstractUnit>, Integer> damages,
                         boolean canAttackAfterMove) {
        this(name, maximumAmmo, 1, 1, damages, canAttackAfterMove);
    }

    public int getAmmunition() { return ammo; }

    public int getMaximumAmmunition() { return maximumAmmo; }

    public void replenish() { this.ammo = this.maximumAmmo; }
    ...
    public boolean isInRange(AbstractUnit unit, AbstractUnit target) {
        int i = Math.abs(unit.getX() - target.getX()) +
    ...
    public boolean canAttack(AbstractUnit shooter, AbstractUnit target) {
        return target != null && shooter.getPlayer() != target.getPlayer() &&
            ammo > 0 &&
            (canAttackAfterMove
                 ? shooter.isEnabled()
                 : shooter.getMoveQuantity() == shooter.getMaxMoveQuantity()) &&
            canShoot(target) && isInRange(shooter, target);
    }

    /**
     * @return the basic minimum range of the weapon
     */
    public int getBaseMinimumRange() { return minimumRange; }
     ...
     */
    public int getBaseMaximumRange() { return maximumRange; }
    ...
    /**
     * @return true if and only if the weapon is a contact weapon (it attacks
