...
public class StatView extends View {
    ...
    @SuppressWarnings("unused") private StatController controller;
    ...
    static {
        ...
        defeatsImages = new Image[] {
            sp.getImage(92, 1, 12, 14, 2), sp.getImage(92, 32, 12, 14, 2),
            sp.getImage(63, 15, 12, 14, 2), sp.getImage(79, 32, 12, 14, 2)};
    }
    ...
    public static void addDefeat(int i, int turn) { defeats[i] = turn; }
    ...
    private class StatPanel extends JPanel {
        ...
        public StatPanel(int[][] data, boolean lign) {
            ...
            // if every number is 0, we set it to 1 to avoid arithmeticException
            // (divide by 0)
        ...
        public void paintLign(Graphics g, int[][] data, int gap) {
            ...
            for (int i = 0; i < data.length; i++) { // each player
                ...
                for (int j = 1; j < data[i].length; j++) {
                    g.drawLine(
                        x, (max - data[i][j - 1]) * this.getHeight() / max,
                        x + gap, (max - data[i][j]) * this.getHeight() / max);
                    x += gap;
                }
            }
            ...
            for (int i = 0; i < data.length; i++)
                if (StatView.defeats[i] != -1) {
                    ...
                    g.drawImage(
                        StatView.defeatsImages[i], (n - 1) * gap,
                        Math.max(
                            0, Math.min(this.getHeight() - 2 -
                                            StatView.defeatsImages[i].getHeight(
                                                StatView.this),
                                        (max - data[i][n - 1]) *
                                                this.getHeight() / max -
                                            StatView.defeatsImages[i].getHeight(
        ...
        public void paintGraph(Graphics g, int[][] data, int gap) {
            ...
            for (int i = 1; i < coords.length; i++) {
                ...
                for (int j = 0; j < coords[i].length; j++)
                    y[j] = y[y.length - 1 - j];
                for (int j = 0; j < coords[i].length; j++)
                    y[y.length - 1 - j] = coords[i][j] * this.getHeight() /
                                          coords[coords.length - 1][j];
            ...
            for (int i = 0; i < data.length; i++)
                if (StatView.defeats[i] != -1) {
                    int n = StatView.defeats[i];
                    g.drawImage(
                        StatView.defeatsImages[i], (2 * n - 1) * gap / 2,
                        Math.max(
                            0, Math.min(
                                   this.getHeight() - 2 -
                                       StatView.defeatsImages[i].getHeight(
                                           StatView.this),
                                   (coords[i][n] + coords[i + 1][n]) *
                                           this.getHeight() /
                                           (2 * coords[coords.length - 1][n]) -
                                       StatView.defeatsImages[i].getHeight(
                                           StatView.this) /
                                           2)),
                        StatView.this);
        ...
        public void paintComponent(Graphics g) {
            ...
            if (lign)
                paintLign(g, data, gap);
            else
                paintGraph(g, data, gap);
        ...
        public void changeGraph() { lign = !lign; }
    }
    ...
    public StatView(StatController controller) {
        ...
        tabs.addTab("Units", null, new StatPanel(controller.units, false),
                    "Evolution of all players units");
        tabs.addTab("Buildings", null,
                    new StatPanel(controller.buildings, true),
                    "Evolution of all players buildings");
        tabs.addTab("Funds", null, new StatPanel(controller.funds, true),
                    "Evolution of all players funds");
        ...
        lign.addActionListener(
            e -> ((StatPanel)tabs.getSelectedComponent()).changeGraph());
    }
}
