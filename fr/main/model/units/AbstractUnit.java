...
public interface AbstractUnit extends Serializable {
     ...
     */
    default Point position() { return new Point(getX(), getY()); }
     ...
     */
    default boolean dead() { return getLife() <= 0; }
     ...
     */
    default void addLife(int life) { setLife(getLife() + life); }

    /**
     * @param life is the amount of life to remove
     * @return true if and only if the unit is still alive
     */
    default boolean removeLife(int life) { return setLife(getLife() - life); }
    ...
    /**
     * @param fog is a representation of the map, true if the tile if visible
     *     and false otherwise
     * @param b true to use "linear regression" (some tiles may not be visible
     *     if a mountain is between it and the unit for example)
     * Airunits can see every tiles in their range of vision.
     * For navalunits and landunits, it depends on the height of the terrain
     * compared to the one of the unit, and if the terrain hide its content from
     * the unit
     */
    void renderVision(boolean[][] fog, boolean b);
    default void renderVision(boolean[][] fog) { renderVision(fog, true); }

    /**
     * @param map is the boolean map
     * Sets to true all tiles that can be attacked within one day (move +
     * attack)
     */
    default void renderTargets(boolean[][] map) {
        ...
        int oX = m.offset.x, oY = m.offset.y,
     ...
     * set all tiles of the map that can be attacked (with the main or secondary
     * weapon) from the position (x,y) to true, whatever the unit on it
     */
    ...
    /**
     * @return a move zone which represents the part of the map on which the
     *     unit can go
     */
    default MoveZone getMoveMap() {
        Universe u = Universe.get();
        int move = getMoveQuantity(), posX = getX(), posY = getY(),
            x = Math.min(move, posX), y = Math.min(move, posY),
            width = x + 1 + Math.min(move, u.getMapWidth() - posX - 1),
            height = y + 1 + Math.min(move, u.getMapHeight() - posY - 1);
        ...
        for (int i = 0; i < height; i++)
            for (int j = 0; j < width; j++)
                // creation of the new nodes, by default there is no way to go
                // to it
                map[i][j] =
                    new Node(j, i, canStop(offset.x + j, offset.y + i),
                             moveCost(offset.x + j, offset.y + i), move + 1);
        ...
        // these two sets are used to know which one are already evaluated and
        // which are yet to evaluate the nodes that are to evaluate
        ...
        while (!unsettled.isEmpty()) {
            // we get the closest node (the node for which there is a path with
            // the smallest move quantity needed to go on it)
            Node actual = null;
            for (Node n : unsettled)
                if (actual == null || n.compareTo(actual) < 0)
                    actual = n;
            unsettled.remove(actual);
            // could be quickier if we used priority list or something like that
            // but the distance to the node may change so if we did so we may
            // need to remove some elements and put them back in the list and
            // other operations would take longer

            for (Direction d : Direction.cardinalDirections())
                // we add to the unsettled list all adjacent nodes (except those
                // who are already evaluated)
                if (actual.x + d.x < width && actual.x + d.x >= 0 &&
                    actual.y + d.y < height && actual.y + d.y >= 0) {
                    Node target = map[actual.y + d.y][actual.x + d.x];
                    if (!settled.contains(target) &&
                        actual.lowestCost + target.moveCost <
                            target.lowestCost) {
                        // if this path is shorter, change the move quantity
                        // needed to go on this Node
                        target.previous = d;
        ...
        // the object MoveZone is used to return both the offset and the
        // Node[][] map
    }
     ...
     * @return a list of Direction which is a path to go to the target tile (it
        ...
        for (int i = 0; i <= approx; i++)
            for (int j = 0; j <= i; j++)
                for (int[] t : tab) {
                    ...
                    if (u.isValidPosition(ptt.x, ptt.y) &&
                        map[ptt.y][ptt.x].previous != Direction.NONE) {
                        while ((d = map[ptt.y][ptt.x].previous) !=
                               Direction.NONE) {
                            path.addFirst(d);
                            d.opposed().move(ptt);
                        }
     ...
     * @return true if and only if the unit can attack the one given as a
     *     parameter
     */
    boolean canAttack(AbstractUnit u);
    /**
     * @return true if and only if the unit may attack something (doesn't depend
     *     on the thing)
     */
    boolean canAttack();
     ...
     */
    default void attack(AbstractUnit u) { attack(u, true); }

    /**
     * @param u is the unit we want to attack
     * @param counter is true to allow a counter attack from the unit given in
     *     parameter
     */
     ...
     */
    default boolean isEnabled() { return getMoveQuantity() != 0; }

    /**
     * move the unit to the specified location
     * (unlike setLocation it doesn't really teleport because fuel is spent,
     * plus it changes the map of units of the Universe)
     */
     ...
     */
    default void turnEnds() { setMoveQuantity(getMaxMoveQuantity()); }
    ...
    default Integer moveCost(int x, int y) {
        Universe u = Universe.get();
        if (u.isVisibleOpponentUnit(
                x, y)) // if there is an opponent unit we can't go through
            return null;
        else if (this instanceof NavalUnit &&
                 u.getBuilding(x, y) instanceof
                     Dock) // if this is a ship, it can go in a dock
            return 1;
        else
     ...
     */
    default boolean canStop(Point pt) { return canStop(pt.x, pt.y); }
    ...
    default boolean canStop(int x, int y) {
        Universe u = Universe.get();
        return (u.getUnit(x, y) == null ||
                (u.getUnit(x, y).getPlayer() != getPlayer() &&
                 !u.isVisibleOpponentUnit(x, y))) &&
            ((this instanceof NavalUnit && u.getBuilding(x, y) instanceof
                                               Dock) ||
             u.getTerrain(x, y).canStop(this));
    }

    /**
     * @param attacker is the unit that attacks
     * @param b is the weapon used to attack : true for the primary weapon and
     *     false for the secondary weapon
     * @param defender is the unit that defends
     * @return the damage inflicted by the attacker to the defender with the
     *     weapon
     */
    public static int damage(AbstractUnit attacker, int life_attacker,
                             boolean b, AbstractUnit defender) {
        Weapon w =
            b ? attacker.getPrimaryWeapon() : attacker.getSecondaryWeapon();
        if (w == null || !w.canAttack(attacker, defender))
            return 0;
        ...
        AbstractBuilding building =
            Universe.get().getBuilding(defender.getX(), defender.getY());
        int dTR = Universe.get()
                      .getTerrain(defender.getX(), defender.getY())
                      .getDefense(defender) +
                  (building == null ? 0 : building.getDefense(defender));
        int dHP = defender.getLife();

        // the formula isn't obvious but works nicely
        return Math.min(100, Math.max(0, (d * aCO + r) * aHP *
                                             (2000 - 10 * dCO - dTR * dHP) /
                                             10000000));
    }
     ...
     */
    public static int damage(AbstractUnit attacker, boolean b,
