...
class Node implements Comparator<Node>, Comparable<Node> {
    ...
    public void setNeighbours(Node[][] nodeMap) {
        ...
        int[][] cross =
            MapGenerator.getCross(nodeMap.length, nodeMap[0].length, x, y);
        for (int i = 0; i < cross.length; i++) {
            if (nodeMap[cross[i][0]][cross[i][1]] == null) {
                nodeMap[cross[i][0]][cross[i][1]] =
                    new Node(cross[i][0], cross[i][1]);
            }

            if (!blocks[cross[i][0]][cross[i][1]] &&
                !nodeMap[cross[i][0]][cross[i][1]].isBlock())
    ...
    public boolean is(Node n) { return n == this || n.x == x && n.y == y; }
    ...
    public static void setWeights(int[][] w) { weights = w; }
    ...
    public void setBlock() { this.isBlock = true; }

    public void setPassable() { this.isBlock = false; }

    public static void setBlocks(boolean[][] blocks) { Node.blocks = blocks; }
}
