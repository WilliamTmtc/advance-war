...
public class EditorController extends Controller {
    ...
    public static int b = 60, h = b / 3;

    public static final int[][][] refs = new int[][][] {{// right
                                                         {0, 0, h},
                                                         {0, b, b / 2}},
                                                        {// bottom
                                                         {0, b / 2, b},
    ...
    public static final int BEACH = 0, BRIDGE = 1, HILL = 2, LOWLAND = 3,
                            MOUNTAIN = 4, RIVER = 5, ROAD = 6, WOOD = 7,
                            REEF = 8, SEA = 9;
    ...
    public class Adaptater implements ChangeListener {
        ...
        public Adaptater(JSlider width, JSlider height, JSlider seed,
                         JSlider land, JSlider moutain, JSlider wood) {
            this.width = width;
            this.height = height;
            this.seed = seed;
            this.land = land;
        ...
        public void stateChanged(ChangeEvent e) {
            generate(width.getValue(), height.getValue(), seed.getValue(),
                     land.getValue(), moutain.getValue(), wood.getValue());
        }
    }
    ...
    public void generate(int width, int height, int seed, int l, int m, int w) {
        ...
        world = new MapRenderer(
            new Universe.Board(units, null, map, buildings, null, null, 0));
    ...
    public void update() {
        ...
        if (!isListening && press) {
            if (arrowButtons[0].contains(pt))
                ...
                if (t != null)
                    world.setTerrain(
                        t, new Point((int)(camera.getX() + mouse.getX()),
        ...
        if (view != null) {
            ...
            int[][] basePoints =
                new int[][] {{m2, m3}, {m1, m4}, {m1, 20}, {20, m3}};

            for (int i = 0; i < 4; i++) {
                arrowButtons[i].setBounds(basePoints[i][0], basePoints[i][1], b,
                                          b);
                for (int j = 0; j < 2; j++)
                    for (int k = 0; k < 3; k++)
