...
public class Mech extends Unit implements WalkingUnit, CaptureBuilding {
    ...
    public Mech(Player p, int x, int y) { this(p, new Point(x, y)); }

    public Mech(Player player, Point point) {
        super(player, point, WalkingUnit.FUEL_NAME, 99, false, MoveType.MECH, 2,
              2,
              new PrimaryWeapon(PRIMARYWEAPON_NAME, 3, PRIMARYWEAPON_DAMAGES,
                                true),
              SECONDARYWEAPON, NAME, PRICE);
    }

    public boolean canCapture(AbstractBuilding b) {
        return isEnabled() && b instanceof OwnableBuilding &&
            ((OwnableBuilding)b).getOwner() != getPlayer();
    }
}