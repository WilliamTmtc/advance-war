...
public class GameController extends Controller {
    ...
    public enum Mode {
        ...
        private Mode(boolean moveable) { this.moveable = moveable; }
    ...
    private class MainActionPanel extends ControllerPanel {
        ...
        public MainActionPanel() {
            ...
            new Index("Finish turn", () -> { world.next(); });
            ...
            new Index("Options",
                      ()
                          -> MainFrame.setScene(
                              new OptionController(GameController.this)));
        ...
        @Override
        public void onOpen() {
            actions.get(3).setActive(
                world.getCurrentPlayer().getCommander().canActivate(true));
            actions.get(4).setActive(
                world.getCurrentPlayer().getCommander().canActivate(false));
            super.onOpen();
        }
    }
    ...
    public class UnitActionPanel extends ControllerPanel {
        ...
        public UnitActionPanel() {
            ...
            new Index("Capture", () -> {
                if (targetUnit.getX() == unitCursor.getX() &&
                    ...
                    AbstractBuilding b =
                        world.getBuilding(targetUnit.getX(), targetUnit.getY());
            ...
            new Index("Hide", () -> { ((HideableUnit)targetUnit).hide(); });
            new Index("Reveal", () -> { ((HideableUnit)targetUnit).hide(); });
        ...
        public void onOpen() {
            ...
            if (!targetUnit.isEnabled())
                return;

            // enable options associated with the unit
            AbstractBuilding building =
                world.getBuilding(unitCursor.getX(), unitCursor.getY());
            if (targetUnit.canAttack())
                actions.get(2).setActive(true);
            if (targetUnit instanceof CaptureBuilding)
                if (((CaptureBuilding)targetUnit).canCapture(building))
                    actions.get(3).setActive(true);
                else if (building instanceof MissileLauncher &&
                         !((MissileLauncher)building).isFired())
                    actions.get(4).setActive(true);
            if (targetUnit instanceof SupplyUnit)
                actions.get(5).setActive(true);
            if (targetUnit instanceof HealerUnit &&
                ((HealerUnit)targetUnit).canHeal())
                actions.get(6).setActive(true);
            if (targetUnit instanceof HideableUnit)
                if (((HideableUnit)targetUnit).hidden())
                    actions.get(8).setActive(true);
                else
                    actions.get(7).setActive(true);

            for (Direction d : Direction.cardinalDirections()) {
                AbstractUnit u = world.getUnit(targetUnit.getX() + d.x,
                                               targetUnit.getY() + d.y);
                if (u instanceof TransportUnit &&
                    ((TransportUnit)u).canCharge(targetUnit)) {
                    actions.get(9).setActive(true);
                    break;
                }
            }
            if (targetUnit instanceof TransportUnit &&
                ((TransportUnit)targetUnit).getOccupation() != 0)
    ...
    public GameController(String mapName) { this(Universe.restaure(mapName)); }

    public GameController(String mapName, Player ps[]) {
        this(Universe.restaure(mapName).setPlayers(ps));
    }

    public void onOpen() { dayPanel.setVisible(true); }

    /**
     * Represents the controls : a control is an action; differents keys can be
    ...
    public static enum Controls {
        ...
        private static final HashMap<Integer, Controls> commands =
            new HashMap<Integer, Controls>();
        ...
        public void add(int i) { commands.putIfAbsent(i, this); }

        /**
         * Reads the "parameter" file to update the controls ; set default
         * values if the file is incorrect
         */
        public static void updateAll() {
            boolean b = true;
            if (parameters.exists() && parameters.isFile() &&
                parameters.canRead() && parameters.canWrite()) {
                b = false;
        ...
        public static Controls getControl(int i) { return commands.get(i); }
    }
    ...
    private void validUnitMove() {
        ...
        new Thread(() -> { // apply the movement
            world.clearTarget();
            UnitRenderer.Render targetRender =
                UnitRenderer.getRender(targetUnit);
    ...
    private void validUnitTarget() {
        if (world.isEnabled(unitCursor.getX(), unitCursor.getY())) {
            ...
            if (targetUnit.canAttack(target)) {
                int aLife = targetUnit.getLife(), tLife = target.getLife();
                targetUnit.attack(target);
                UnitRenderer.getRender(targetUnit).attackSound();
                world.flash(
                    "" + (targetUnit.getLife() - aLife),
                    (targetUnit.getX() - camera.getX() + 1) * MainFrame.UNIT +
                        5,
                    (targetUnit.getY() - camera.getY()) * MainFrame.UNIT + 5,
                    1000, UniverseRenderer.FlashMessage.Type.ALERT);
                world.flash(
                    "" + (target.getLife() - tLife),
    ...
    private void validLoad() {
        AbstractUnit target = world.getUnit(unitCursor.position());
        if (target instanceof TransportUnit &&
            ((TransportUnit)target).canCharge(targetUnit))
    ...
    private void validUnloadLocate() {
        ((TransportUnit)targetUnit)
            .remove(getTransportUnit(), unitCursor.getX(), unitCursor.getY());
        mode = Mode.MOVE;
        world.clearTarget();
    }
    ...
    private void validMissileLauncher() {
        fireMissile((MissileLauncher)world.getBuilding(targetUnit.getX(),
                                                       targetUnit.getY()),
    ...
    public void findNextUnit() {
        ...
        if (!units.isEmpty()) {
            int n = (units.indexOf(unit) + units.size()) % units.size(),
                // indexOf returns -1 if the object is not in the list, here we
                // want to have the last element in this case
                i = (n + 1) % units.size();
        ...
        if (!isListening) {
            ...
            if (mode.canMove()) {
                ...
                if (key == Controls.MOVE_TOP)
                    ...
                    validRange();
                } else if (key == Controls.NEXT_UNIT && mode == Mode.MOVE)
                    findNextUnit();
            } else if (mode == Mode.MENU) { // update index and valid menu
                                            // action for focusedActionPanel
                if (key == Controls.MOVE_TOP)
                    focusedActionPanel.goUp();
                else if (key == Controls.MOVE_BOTTOM)
                    focusedActionPanel.goDown();
                else if (key == Controls.ENTER)
                    focusedActionPanel.perform();
            }
    ...
    public void mouseWheelMoved(MouseWheelEvent e) {
        if (mode == Mode.MENU) { // change index in focusedActionPanel
            Runnable action = e.getWheelRotation() < 0
                                  ? focusedActionPanel::goUp
                                  : focusedActionPanel::goDown;
    ...
    public void update() {
        ...
        isListening =
            cursor.move() | camera.move() |
            (mode != Mode.MOVE && mode.canMove() && unitCursor.move());
        if (!isListening && mode == Mode.MISSILE_LAUNCHER)
            world.updateTarget(unitCursor.position());

        displayDamages = false;

        if (!isListening && mode == Mode.ATTACK &&
            world.isVisibleOpponentUnit(unitCursor.position())) {
            AbstractUnit attacker = world.getUnit(cursor.position()),
                         defender = world.getUnit(unitCursor.position());
            if (!point2.equals(cursor.position()))
                point1.move(-1, -1);
            if (attacker.canAttack(defender) &&
                !point1.equals(unitCursor.position())) {
                point1 = unitCursor.position(); // the target
                point2 = cursor.position();     // the selected unit
                damage1 = AbstractUnit.damage(
                    attacker,
                    attacker.getPrimaryWeapon() != null &&
                        attacker.getPrimaryWeapon().canAttack(attacker,
                                                              defender),
                    defender);
                damage2 = AbstractUnit.damage(
                    defender, defender.getLife() - damage1,
                    defender.getPrimaryWeapon() != null &&
                        defender.getPrimaryWeapon().canAttack(defender,
                                                              attacker),
                    attacker);
            }
            if (attacker.canAttack(defender))
        ...
        if (!isListening && mode.canMove() && listenMouse) {
            if (camera.getX() > 0 && mouse.x <= moveRange)
                camera.setDirection(Direction.LEFT);
            else if (camera.getX() + camera.width < size.width &&
                     camera.width - mouse.x <= moveRange)
                camera.setDirection(Direction.RIGHT);
            else if (mouse.y <= moveRange)
                camera.setDirection(Direction.TOP);
            else if (camera.height - mouse.y <= moveRange)
                camera.setDirection(Direction.BOTTOM);

            Position.Cursor c =
                mode == Mode.MOVE || mode == Mode.MENU ? cursor : unitCursor;
    ...
    private void move(Direction d) {
        ...
        if ((d == Direction.LEFT && c.getX() - camera.getX() <= moveRange) ||
            (d == Direction.RIGHT &&
             camera.getX() + camera.width - c.getX() <= moveRange + 1) ||
            (d == Direction.TOP && c.getY() - camera.getY() <= moveRange) ||
            (d == Direction.BOTTOM &&
             camera.getY() + camera.height - c.getY() <= moveRange + 1))
    ...
    private void displayDamages(Graphics g, int damage, Point location) {
        int x = (location.x - camera.getX()) * MainFrame.UNIT,
            y = (location.y - camera.getY()) * MainFrame.UNIT -
                (location.y != 0 ? MainFrame.UNIT / 2 : 0);
        g.drawImage(damageImage, x, y, gameViewController);
        g.drawImage(numbers[damage / 100], x + 1, y + 10, gameViewController);
        g.drawImage(numbers[(damage / 10) % 10], x + 9, y + 10,
                    gameViewController);
        g.drawImage(numbers[damage % 10], x + 16, y + 10, gameViewController);
    }

    /**
     * Display damages inflicted by the unit to the target and the counter
     * attack
     */
    ...
    /**
     * True if and only if possible damages are displayed (in attack mode, when
     * hovering an opponent unit)
     */
