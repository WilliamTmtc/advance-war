...
public class Path extends LinkedList<Direction> {
    ...
    public static Path getPath() { return instance; }
    ...
    /**
     * Delete the whole path and replace it by the shortest way to go to the
     * last point
     */
    public void shorten() {
    ...
    protected void shorten(Point p) {
        ...
        if (p.x < 0 || p.y < 0 || p.x >= map[0].length || p.y >= map.length)
            // if it is not a point of the relative map, there is no way to go
            // to it
            return;
        clear();
        points.clear();
        ...
        // the strategy here is to begin at the target and to go back tile by
        // tile until we reached the unit's position
        Direction d;
        while ((d = map[p.y][p.x].previous) != Direction.NONE) {
            // the position of the unit is the only accessible tile whose
            // 'previous' direction is NONE so if the tile is accessible, and
            // its previous direction is NONE, it means we arrived onto the unit
            // original position
            addFirst(d);
            d.opposed().move(p);
     ...
     * @return true if and only if the unit can stop on the point and the point
     *     is not the last one
    ...
    public boolean add(Point p) {
        ...
        p.translate(-offset.x, -offset.y);
        if (p.x >= 0 && p.y >= 0 && p.x < map[0].length && p.y < map.length) {
            // if it is a point of the relative map, we try to add it to the
            // path
            if (points.getLast().equals(p))
                return false; // the point is the last one, so the list isn't
                              // modified
            Direction d = Direction.getDir(
                points.getLast(), p); // the direction to go from the last point
                                      // to the argument point
            // it is NONE if these point aren't adjacents
            if (map[p.y][p.x].canStop &&
                map[p.y][p.x].lowestCost <= unit.getMoveQuantity())
                // if the unit can go on the tile, and the tile is adjacent, we
                // add it if the tile isn't adjacent, we use the shortest path
                // to go to it
                if (d == Direction.NONE) {
                    p.translate(offset.x, offset.y);
                    shorten(p);
    ...
    public boolean add(Direction dir) {
        ...
        dir.move(t);
        // we consider the point we want to add (which is the last point move
        // according to the parameter direction)

        if (!map[t.y][t.x].canStop)
            return false;
        ...
        if (i != -1) {
            while (points.size() != i + 1) {
                removeLast();
                pathMoveCost -=
                    map[points.getLast().y][points.getLast().x].moveCost;
                points.removeLast();
            }
            return true;
        }

        // if the move quantity needed to go to the point going by the path is
        // too high, we shorten the path (replace it by the shortest)
     ...
     */
    public boolean apply() { return unit.move(this); }
}
