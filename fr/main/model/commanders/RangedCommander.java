...
public class RangedCommander extends Commander {
    ...
    public int getVision(AbstractUnit u) {
        return u.getBaseVision() +
            Universe.get().getTerrain(u.getX(), u.getY()).getBonusVision(u) +
            (u.getPrimaryWeapon() != null &&
                     !u.getPrimaryWeapon().isContactWeapon()
                 ? 1
                 : 0) *
                (activated(true) ? 2 : (activated(false) ? 1 : 0));
    }

    public int getMaximumRange(AbstractUnit u, PrimaryWeapon p) {
        return p.isContactWeapon() ? 1
                                   : p.getBaseMaximumRange() +
                                         Universe.get()
