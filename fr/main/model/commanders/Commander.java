 ...
 * A commander has at most two powers, he has a power bar that charges when he
 * takes damages and he can improve or deteriorate any characteristic of any
 * unit as a passive or active power.
 */
public abstract class Commander implements Serializable {
    ...
    /**
     * The power bar of the commander, showing how much points he has to
     * activate its powers
     */
    public class PowerBar implements Serializable {
        ...
        public void addValue(int i) { value = Math.min(maxValue, value + i); }
        public void removeValue(int i) { value = Math.max(0, value - i); }
    }
    ...
    protected static class Power implements Serializable {
        ...
        public Power(int value) { this(value, p -> {}); }
    ...
    public Commander(Player player, Power small, Power big) {
        ...
        this.big = big;
        this.player = player;
        int bar = Math.max(small == null ? 0 : small.value,
                           big == null ? 0 : big.value);
        this.powerBar = bar == 0 ? null : new PowerBar(bar);
    }

    /**
     * @param bigPower is true to activate the big power and false for the small
     *     one
    ...
    /**
     * @param bigPower is true to return if the big power is activated and false
     *     for the small one
    ...
    /**
     * @param bigPower is true to return if the big power can be activated and
     *     false for the small one
     * @return true if and only if the matching power can be activated
     */
    public boolean canActivate(boolean bigPower) {
        return !activated(true) && !activated(false) &&
            powerBar.value >= getPowerCost(bigPower);
    }

    /**
     * @param bigPower is true to return the big power's cost and false for the
     *     small one
     ...
     * @return the attack value (as a percentage) of the commander for the given
     *     unit
     */
    public int getAttackValue(AbstractUnit u) { return 100; }

    /**
     * @param u is the unit we want to know the defense value
     * @return the defense value (as a percentage) of the commander for the
     *     given unit
     */
    public int getDefenseValue(AbstractUnit u) { return 100; }
    ...
    public int getVision(AbstractUnit u) {
        return Math.max(1, u.getBaseVision() +
                               Universe.get()
                                   .getTerrain(u.getX(), u.getY())
                                   .getBonusVision(u) -
                               Universe.get().getWeather().malusVision);
    }
    ...
    public int getMaximumRange(AbstractUnit u, PrimaryWeapon p) {
        return p.isContactWeapon()
            ? 1
            : p.getBaseMaximumRange() + Universe.get()
                                            .getTerrain(u.getX(), u.getY())
                                            .getBonusRange(u, p);
    }
}
