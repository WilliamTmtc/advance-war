...
public class UnitControlAI implements ArtificialIntelligence {
    ...
    /**
     * When a headQuarter is captured, it is changed in a city, so the
     * objectives list has to be updated
    ...
    /**
     * Finds all the buildings of the map from the closest of the headquarter to
     * the farthest
    ...
    private void findObjectives(Headquarter hq) {
        ...
        for (int i = 1; bool; i++) {
            ...
            for (int j = 1; j < i; j++)
                for (int[] t : tab) {
                    bool = bool ||
                           u.isValidPosition(x + j * t[0], y + (i - j) * t[1]);
                    AbstractBuilding b =
                        u.getBuilding(x + j * t[0], y + (i - j) * t[1]);
                    if (b != null && b instanceof OwnableBuilding)
                        objectives.add((OwnableBuilding)b);
                }
        }
    ...
    public void add(AbstractUnit u) { units.put(u, new UnitActionChooser(u)); }

    public void remove(AbstractUnit u) { units.remove(u); }

    public void run() {
        // we have to use a copy of the list to avoid concurrent modification
        // exception if an unit dies for example
        ArrayList<UnitActionChooser> unitList =
            new ArrayList<UnitActionChooser>(units.values());

        // first ranged units play
        checkUnits(unitList.iterator(),
                   u
                   -> u.getPrimaryWeapon() != null &&
                          !u.getPrimaryWeapon().isContactWeapon());

        // then contact units play
        checkUnits(unitList.iterator(),
                   u
                   -> u.getPrimaryWeapon() != null ||
                          u.getSecondaryWeapon() != null);
        ...
        // because the situation may have changed since the first calculus of
        // the thing to do
        checkUnits(unitList.iterator(), u -> true);

        if (Universe.get().getDay() % 10 == 0)
            for (UnitActionChooser uac : units.values())
                if (uac.getState() == UnitActionChooser.State.DEFEND ||
                    uac.getState() == UnitActionChooser.State.ATTACK ||
                    uac.getState() == UnitActionChooser.State.AIMLESS)
                    uac.findState();
    }
     ...
     * this method checks all units of the iterator that pass the test, finds
     * the good action and run it
     */
    private void checkUnits(Iterator<UnitActionChooser> iterator,
                            Predicate<AbstractUnit> test) {
        while (iterator.hasNext()) {
            ...
            if (!actionChooser.unit.isEnabled())
                iterator.remove();
        }
    }
