...
public abstract class Unit implements AbstractUnit {
    ...
    public class Fuel implements java.io.Serializable {
        ...
        public final String name; // l'infanterie n'a pas de 'carburant' mais
                                  // des 'rations' (c'est un détail mais bon)
        ...
        public void replenish() { this.quantity = this.maximumQuantity; }
        public int getQuantity() { return quantity; }
    }

    public Unit(Player player, Point location, String fuelName, int maxFuel,
                boolean diesIfNoFuel, MoveType moveType, int moveQuantity,
                int vision, PrimaryWeapon primaryWeapon,
                SecondaryWeapon secondaryWeapon, String name, int cost) {
        this.life = 100;
        this.player = player;
    ...
    public final boolean move(int x, int y) {
        ...
        if (u.isValidPosition(x, y)) {
            ...
            if (unit != null && unit.getPlayer() != getPlayer()) {
                // if there is an unit on the tile we want to move to, and if it
                // is an opponent, it attacks this unit, and there is no counter
                // attack possible
                if (unit.canAttack(this))
                    unit.attack(this, false);
                setMoveQuantity(0);
                return false;
            }

            Integer q = moveCost(x, y);
            if (q == null || q > getMoveQuantity()) { // the cost to move on the
                                                      // tile is too high
                setMoveQuantity(0);
                return false;
            }
            ...
            // it should be 1 but in case we teleport the unit, we spend the
            // right amount
            int fuelQuantity =
    ...
    public final void renderVision(boolean[][] fog, boolean linearRegression) {
        int x = location.x, y = location.y, visionT = getVision(),
            height = this instanceof AirUnit
                         ? 2
                         : Universe.get().getTerrain(x, y).getHeight();

        fog[y][x] = true; // the current tile can always be seen
        if (visionT != 0) {
            // we decompose the tiles in 2 categories according to the position
            // of the unit (to avoid checking multiple times the same tile)
            // first we check the tiles that are in straight line
            for (int i = 1; i <= visionT; i++)
                for (Direction d : Direction.cardinalDirections()) {
                    int xx = x + i * d.x, yy = y + i * d.y;
                    if (xx >= 0 && yy >= 0 && yy < fog.length &&
                        xx < fog[yy].length && !fog[yy][xx] &&
                        (i == 1 ||
                         !Universe.get().getTerrain(xx, yy).hideFrom(this)) &&
                        (!linearRegression || height == 2 ||
                         linearRegression(x, y, xx, yy, height)))
                        fog[yy][xx] = true;
                }
     ...
     * @return true if and only if there is nothing blocking the vision of the
     *     unit between its position and the target tile
     */
    private boolean linearRegression(int startX, int startY, int x, int y,
                                     int height) {
        ...
        if (Math.abs(x) + Math.abs(y) <= 2)
            return true; // the two tiles are adjacents

        // we use two int to know if the move is oriented to the right or the
        // left and the top or the bottom
        int epsilonX = x > 0 ? 1 : -1, epsilonY = y > 0 ? 1 : -1;
        // we use two points (a,b) and (c,d) to represent the tile we are
        // considering (from the beginning and from the end : the line is
        // symmetrical)
        int a = 0, b = 0, c = x, d = y;

        // we look what direction is the most used : if the move is to the upper
        // right for example, if the move is mainly horizontal then the first
        // move to do is to go to the right else if the move is mainly vertical
        // then the first move to do is to go to the top and otherwise it means
        // that the move is as much vertical as horizontal and so it is a
        // diagonal move then we have to move diagonaly we go to the next tile
        ...
        // if the move was diagonal then we check if one of the two tiles
        // possibles : if we're going from the tile q to the tile i, we check if
        // we can see through tiles l or r, tile m, and tile h or n
        /* a b c d e
           f g h i j
           k l m n o
           p q r s t
        ...
        */
        if (x * x == y * y) { // diagonal path, check first and last tiles
            if (!canSeeThrough(startX, startY, 2 * epsilonX, 0, height) &&
                !canSeeThrough(startX, startY, 0, 2 * epsilonY, height))
                return false;
            if (!canSeeThrough(startX, startY, x - 2 * epsilonX, y, height) &&
                !canSeeThrough(startX, startY, x, y - 2 * epsilonY, height))
                return false;
        }

        // we check the whole straight line
        while (a * epsilonX < c * epsilonX || b * epsilonY < d * epsilonY) {
            if (!canSeeThrough(startX, startY, a, b, height) ||
                !canSeeThrough(startX, startY, c, d, height)) // check actual
                return false;

            if (y * (a + epsilonX) == x * (b + epsilonY)) { // diagonal
                if (!canSeeThrough(startX, startY, a + 2 * epsilonX, b,
                                   height) &&
                    !canSeeThrough(startX, startY, a, b + 2 * epsilonY, height))
                    return false;

                if (!canSeeThrough(startX, startY, c - 2 * epsilonX, d,
                                   height) &&
                    !canSeeThrough(startX, startY, c, d - 2 * epsilonY, height))
                    return false;
            }

            // we use these booleans to know if we move verticaly or horizontaly
            // we check if the corner a tile is above or below the straight line
            // (the corner corresponding to the directions of the move) if both
            // are true then the move is diagonal
            boolean sup = epsilonX * epsilonY * y * (a + epsilonX) >=
                          epsilonX * epsilonY * x * (b + epsilonY),
                    inf = epsilonX * epsilonY * y * (a + epsilonX) <=
                          epsilonX * epsilonY * x * (b + epsilonY);
            if (sup) { // vertical
                b += 2 * epsilonY;
                d -= 2 * epsilonY;
    ...
    private final boolean canSeeThrough(
        ...
        int height) { // especially written for linearRegression, should
                      // probably not be used in any other method
        AbstractTerrain t =
            Universe.get().getTerrain(x / 2 + startX, y / 2 + startY);
        return t.hideFrom(this) ? t.getHeight() < height
                                : t.getHeight() <= height;
    }
    ...
    public String toString() {
        ...
        if (fuel != null)
            out += "\n" + fuel.name + " : " + fuel.getQuantity() + "/" +
                   fuel.maximumQuantity;
        if (primaryWeapon != null) {
            out += "\n" + primaryWeapon.name + " : " +
                   primaryWeapon.getAmmunition() + "/" +
                   primaryWeapon.maximumAmmo;
            if (!primaryWeapon.isContactWeapon())
                out += ", " + primaryWeapon.minimumRange + "~" +
                       primaryWeapon.maximumRange;
        }
    ...
    public boolean canAttack(AbstractUnit u) {
        return (primaryWeapon == null ? false
                                      : primaryWeapon.canAttack(this, u)) ||
            (secondaryWeapon == null ? false
                                     : secondaryWeapon.canAttack(this, u));
    }

    @Override
    public boolean canAttack() {
        return (primaryWeapon != null &&
                (getMoveQuantity() == getMaxMoveQuantity() ||
                 primaryWeapon.canAttackAfterMove)) ||
