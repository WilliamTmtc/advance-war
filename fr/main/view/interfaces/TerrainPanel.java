...
public class TerrainPanel extends InterfaceUI {
    ...
    protected void draw(Graphics g) {
        ...
        leftSide = cursor.getX() - camera.getX() >= halfw &&
                   cursor.getY() - camera.getY() >= halfh;
        ...
        AbstractTerrain terrain =
            world.getTerrain(cursor.getX(), cursor.getY());
        AbstractUnit unit = world.getUnit(cursor.getX(), cursor.getY());
        AbstractBuilding building =
            world.getBuilding(cursor.getX(), cursor.getY());

        // image displayed : terrain or building
        Point img = new Point(x + 30, y + 20);
        if (building == null)
            TerrainRenderer.render(g, img,
                                   cursor.position()); // display the building
        else
            BuildingRenderer.render(g, img, building); // display the terrain
        ...
        g.drawString(name,
                     x + 30 + MainFrame.UNIT / 2 -
                         g.getFontMetrics().stringWidth(name) / 2,
                     y + 70);

        // terrain defense
        int defense = terrain.getDefense(null) +
                      (building == null ? 0 : building.getDefense(null));
        ...
        if (unit != null &&
            ...
            g.drawImage(UnitRenderer.Render.getMoveImage(unit), x + 15, y + 153,
                        null);
            g.drawString(unit.getMoveQuantity() + "/" +
                             unit.getMaxMoveQuantity(),
                         x + 38, y + 158);
            ...
            if (p != null) {
                ...
                g.drawString(p.getAmmunition() + "/" + p.maximumAmmo, x + 75,
                             y + 158);
            }
            ...
            g.drawString(fuel.getQuantity() + "/" + fuel.maximumQuantity,
                         x + 35, y + 185);
            ...
            if (building instanceof OwnableBuilding) {
                ...
                // building's owner
                g.drawString(p == null ? "Neutre" : p.name, x + 15,
                             y + 120); // owner

                g.drawImage(buildingImage, x + 15, y + 130, null);
                // building's life
                g.drawString(((OwnableBuilding)building).getLife() + "", x + 45,
                             y + 145);
            }
