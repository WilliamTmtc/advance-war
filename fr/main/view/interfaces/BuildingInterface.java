...
public class BuildingInterface extends GameController.ControllerPanel {
    ...
    class IndexClass extends Index {
        ...
        public IndexClass(Class<? extends AbstractUnit> c)
            ...
            super(c.getField("NAME").get(null) + ": " +
                      c.getField("PRICE").get(null) + "$",
                  () -> building.create(c));
            this.c = c;
        }
    }
    ...
    public BuildingInterface(GameController controller) {
        ...
        try {
            ...
            Comparator<Class<? extends AbstractUnit>> comparator =
                Comparateur.instance;

            list =
                new ArrayList<Class<? extends AbstractUnit>>(Dock.getUnits());
            Collections.sort(list, comparator);
            for (Class<? extends AbstractUnit> c : list)
                new IndexClass(c);

            list = new ArrayList<Class<? extends AbstractUnit>>(
                Barrack.getUnits());
            Collections.sort(list, comparator);
            for (Class<? extends AbstractUnit> c : list)
                new IndexClass(c);
     ...
     */
    static class Comparateur
        implements Comparator<Class<? extends AbstractUnit>> {
        ...
        public int compare(Class<? extends AbstractUnit> o1,
                           Class<? extends AbstractUnit> o2) {
            try {
                return (Integer)o1.getField("PRICE").get(null) -
                    (Integer)o2.getField("PRICE").get(null);
    ...
    public void onOpen() {
        super.onOpen();
        building = (FactoryBuilding)controller.world.getBuilding(
            controller.cursor.position());
