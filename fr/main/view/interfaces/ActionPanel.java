...
public class ActionPanel extends InterfaceUI {
     ...
     * Select first action
     */
    public void moveTop() { selected = 1; }

    /**
     * Select the action above the current selected
     */
    public void goUp() { selected = Math.max(1, selected - 1); }

    /**
     * Select the action under the current selected
     */
    public void goDown() { selected = Math.min(actives, selected + 1); }
    ...
    public void onOpen() {
        ...
        for (InterfaceUI com : InterfaceUI.components())
            if (!(com instanceof ActionPanel) && hideOnOpen(com))
    ...
    public void onClose() {
        for (InterfaceUI com : InterfaceUI.components())
            if (!(com instanceof ActionPanel) && showOnClose(com))
                com.setVisible(true);
    }
