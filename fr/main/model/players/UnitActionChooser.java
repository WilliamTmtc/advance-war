 ...
 * Each instance is an FSM (finite state machine) which manages one unit (with
 * total independence with other units)
 */
public class UnitActionChooser implements java.io.Serializable {
     ...
     */
    public static enum State implements java
    .io.Serializable {
        AIMLESS(u -> u::aimless), // goes by the map without clear objective
            REPLENISH(
                u -> u::replenish), // bring the unit on a tile it can replenish
                                    // (get fuel, ammo & get healed)
            DEFEND(
                u -> u::defend), // defend a specific tile or building or unit
            ATTACK(u -> u::attack),       // attack opponents
            OBJECTIVE(u -> u::objective); // an objective is a tile, a building
                                          // or an unit this unit wants to reach
                                          // once the objective is reached, a
        ...
        public void apply(UnitActionChooser u) { action.apply(u).run(); }
        ...
        public int attack(UnitActionChooser u, Point move,
                          AbstractUnit attacked) {
            switch (u.getState()) {
            case AIMLESS:
                return 0;
            case REPLENISH:
    ...
    public UnitActionChooser(AbstractUnit unit) {
        ...
        this.indirect = unit.getPrimaryWeapon() != null &&
                        !unit.getPrimaryWeapon().isContactWeapon();
    ...
    /**
     * finds the state the unit should be in according to what is around it and
     * what is on the map
     */
    public void findState() {
        if (unit.getLife() < 30 || unit.getFuel().getQuantity() < 25 ||
            (unit.getPrimaryWeapon() != null &&
             unit.getPrimaryWeapon().getAmmunition() < 3)) {
            for (OwnableBuilding b : unit.getPlayer().buildingList())
                if (b instanceof RepairBuilding &&
                    ((RepairBuilding)b).canRepair(unit)) {
                    LinkedList<Direction> path =
                        unit.findPath(b.getLocation(), 3);
                    if (!path.isEmpty()) {
                        final Objective o =
                            new Objective(b.getLocation(), 0, path);
                        o.setAction(() -> {
        ...
        for (OwnableBuilding b :
             ((AIPlayer)unit.getPlayer()).unitControlAI.getObjectives())
            if (b.getOwner() != unit.getPlayer()) {
                LinkedList<Direction> path = unit.findPath(b.getLocation(), 3);
                if (!path.isEmpty()) {
                    target = b.getLocation();
                    objective = new Objective(b.getLocation(), 3,
                                              () -> state = State.ATTACK, path);
                    state = State.OBJECTIVE;
                    return;
                }
            }
        ...
        OwnableBuilding b =
            (new ArrayList<OwnableBuilding>(unit.getPlayer().buildingList()))
                .get((new Random())
                         .nextInt(unit.getPlayer().buildingList().size()));
    ...
    private class Objective implements java.io.Serializable {
         ...
         * If approx == 1, then if the unit is on a tile next to the target it
         * is considered that the unit reached the target
         */
        ...
        /**
         * The timer is used to check that an objective is reachable, once the
         * timer is 0 then the objective is re-calculated
         */
        private int timer;
        /**
         * The path to follow to go to the objective, can be checked sometimes
         * to be sure it is still valid
         */
        ...
        public Objective(Point target, int approx, Runnable todo,
                         LinkedList<Direction> path) {
            this.target = target;
            this.approx = approx;
            this.todo = todo;
        ...
        public void decreaseTimer() {
            ...
            if (timer <= 0) {
                path.clear();
                path.addAll(
                    UnitActionChooser.this.unit.findPath(target, approx));
                setTimer(5);
            }
        }
        ...
        public void setAction(Runnable r) { todo = r; }
        ...
        public int attack(Point p, AbstractUnit attacked) { return 10; }

        public int heal(Point p, AbstractUnit healed) { return 10; }

        public int supply(Point p) { return 10; }

        public int capture(OwnableBuilding b) { return 500; }
    }
    ...
    public void aimless() { action(); }

    /**
     * Runnable doing what its name indicates : take the unit to a building
     * where it may replenish
     */
    public void replenish() { action(); }

    /**
     * Runnable doing what its name indicates : defend a position
     */
    public void defend() { action(); }

    /**
     * Runnable doing what its name indicates : attack a position
     */
    public void attack() { action(); }
    ...
    public void objective() {
        ...
        if (unit.isEnabled() && Math.abs(unit.getX() - target.x) +
                                        Math.abs(unit.getY() - target.y) <=
                                    objective.getApprox())
            action = () -> {
    ...
    private void action() {
        ...
        boolean attackBis =
            unit.canAttack() && (unit.getPrimaryWeapon() == null ||
                                 unit.getPrimaryWeapon().isContactWeapon());
        ...
        for (int i = 0; i < localMap.length; i++)
            for (int j = 0; j < localMap[i].length; j++)
                if (localMap[i][j].lowestCost < unit.getMoveQuantity()) {
                                ...
                                action = () -> {
                                    ...
                                    if (p.apply()) {
                                        ...
                                        int aLife = unit.getLife(),
                                            tLife = u.getLife();
                                        unit.attack(u);
                                        UnitRenderer.getRender(unit)
                                            .attackSound();
                                        world.flash(
                                            "" + (unit.getLife() - aLife),
                                            (unit.getX() - camera.getX() + 1) *
                                                    MainFrame.UNIT +
                                                5,
                                            (unit.getY() - camera.getY()) *
                                                    MainFrame.UNIT +
                                                5,
                                            1000,
                                            UniverseRenderer.FlashMessage.Type
                                                .ALERT);
                                        world.flash(
                                            "" + (u.getLife() - tLife),
                                            (u.getX() - camera.getX() + 1) *
                                                    MainFrame.UNIT +
                                                5,
                                            (u.getY() - camera.getY()) *
                                                    MainFrame.UNIT +
                            ...
                            action = () -> {
                                ...
                                if (p.apply()) {
                                    ...
                                    for (Direction d :
                                        ...
                                        int xx = unit.getX() + d.x,
                                            yy = unit.getY() + d.y;
                                        AbstractUnit unitBis =
                                            world.getUnit(xx, yy);
                                        if (((SupplyUnit)unit)
                                                .canSupply(unitBis))
                                            world.flash(
                    ...
                    if (capture && ((CaptureBuilding)unit)
                                       .canCapture(world.getBuilding(pt))) {
                        ...
                        if (tmp > actionPoint) {
                            ...
                            action = () -> {
                                ...
                                p.add(pt.getLocation());
                                if (p.apply())
                                    if (((CaptureBuilding)unit)
                                            .capture(building))
                                        BuildingRenderer.getRender(building)
                                            .updateState("");
                            };
                        }
                    }
        ...
        if (indirect && unit.getPrimaryWeapon().getAmmunition() != 0) {
            ...
            for (int i = p.getMinimumRange(unit); i <= p.getMaximumRange(unit);
                 i++)
                for (int j = 0; j <= i; j++)
                    for (int[] d : t) {
    ...
    /* all these methods return how much points it earns to do the corresponding
     * action */
    ...
    private int move(Point move) {
        ...
        for (int i = -5; i <= 5; i++)
            for (int j = -5; j <= 5; j++) {
                ...
                if (aU != null) {
                    if (aU.getPlayer() == unit.getPlayer())
                        p += aU.getCost() / 1000;
                    else if (unit.canAttack(aU)) {
                        if (indirect && aU.canAttack(unit))
                            p -= aU.getCost() / 1000;
                        else {
                            int damage2 = AbstractUnit.damage(
                                    unit,
                                    unit.getPrimaryWeapon() != null &&
                                        unit.getPrimaryWeapon().canAttack(unit,
                                                                          aU),
                                    aU),
                                damage1 = AbstractUnit.damage(
                                    aU, aU.getLife() - damage2,
                                    aU.getPrimaryWeapon() != null &&
                                        aU.getPrimaryWeapon().canAttack(aU,
                                                                        unit),
                                    unit);
                            if (damage2 > damage1)
                                ...
                                p -=
                                    unit.getCost() * (damage2 - damage1) / 1000;
                        }
                    } else if (aU.canAttack(unit))
                        p -= aU.getCost() / 1000;
                }
     ...
     * @return how much points it earns to move to the point and then attack the
     *     unit
     */
    private int attack(Point move, AbstractUnit attacked) {
        int a = AbstractUnit.damage(
            unit,
            unit.getPrimaryWeapon() != null &&
                unit.getPrimaryWeapon().canAttack(unit, attacked),
            attacked);
        int b = AbstractUnit.damage(
            attacked, attacked.getLife() - a,
            attacked.getPrimaryWeapon() != null &&
                attacked.getPrimaryWeapon().canAttack(attacked, unit),
            unit);
        return state.attack(this, move, attacked) + move(move) +
            (a >= b ? 1 : -1) *
                (a * attacked.getCost() / 100 - b * unit.getCost() / 100) +
            (a >= attacked.getLife() ? 10 : 0);
    }

    /**
     * @param move is the tile to which the unit will move
     * @param healed is the unit to heal
     * @return how much points it earns to move to the point and then heal the
     *     unit
     */
    private int heal(Point move, AbstractUnit healed) {
        return state.heal(this, move, healed) + move(move) +
                        unit.getPlayer().getFunds() >=
                    healed.getCost() / 10 &&
                healed.getLife() <= 90
            ? healed.getCost() / 1000
            : 0;
    ...
    private int supply(Point move) {
        ...
        for (Direction d : Direction.cardinalDirections())
            if (((SupplyUnit)unit)
                    .canSupply(
                        Universe.get().getUnit(move.x + d.x, move.y + d.y)))
                n += 5;
     ...
     * @return how much points it earns to move to the building and then
     *     capturing it
     */
    private int capture(OwnableBuilding captured) {
        return state.capture(this, captured) +
            move(new Point(captured.getX(), captured.getY())) +
            (captured instanceof Headquarter
                 ? 1000
                 : (captured instanceof FactoryBuilding ? 500 : 250));
    }

    /**
     * @param move is the tile to which the unit will move
     * @return how much points it earns to move to the point and then hide or
     *     reveal (depends on if the unit is already hidden or not)
     */
    private int hide(Point move) { return 0; }

    /**
     * @param transport is the transport to go in
     * @return how much points it earns to go in the transport
     */
    private int goInTransport(TransportUnit transport) { return 0; }
     ...
     * @return how much points it earns to move to the point and then remove the
     *     unit on the tile specified by the direction
     */
    private int goOutTransport(Point move, AbstractUnit toRemove,
                               Direction dir) {
        return 0;
    }
}