...
public class UniverseRenderer extends MapRenderer {
    ...
    /**
     * A text message displayed for an specific amount of time on a specific
     * position
     */
    ...
    public UniverseRenderer(AbstractUnit[][] units, AbstractTerrain[][] map,
                            Player[] ps, AbstractBuilding[][] buildings,
    ...
    public UniverseRenderer(String mapName, Player[] ps,
                            GameController controller) {
        this(Universe.restaure(mapName).setPlayers(ps), controller);
    }

    public boolean isEnabled(int x, int y) { return targets[y][x]; }
    ...
    public Frame draw(Graphics g, int x, int y, int offsetX, int offsetY) {
        ...
        frame.forEach((i, j) -> {
            if (targets[i][j]) {
                g.setColor(tColor);
                g.fillRect(coords[i][j].x, coords[i][j].y, MainFrame.UNIT,
                           MainFrame.UNIT);
            }
        ...
        while (iterator2.hasNext()) {
            ...
            g.drawImage(
                d.getImage(), (d.x - controller.camera.getX()) * MainFrame.UNIT,
                (d.y - controller.camera.getY()) * MainFrame.UNIT, null);
            d.time--;
            if (d.time <= 0)
                iterator2.remove();
        }
    ...
    public void next() {
        ...
        if (controller != null) {
            if (w != getWeather())
                controller.makeView().getWeatherController().update(w);
            controller.onOpen();
        }
    }
     ...
     * For example in attack mode, it will set to true the tiles that can be
     * attacked
     */
        ...
        if (controller.getMode() == GameController.Mode.UNIT) {
            ...
            for (int j = upperLeft.y; j < lowerRight.y; j++)
                for (int i = upperLeft.x; i < lowerRight.x; i++)
                    targets[j][i] =
                        n[j - upperLeft.y][i - upperLeft.x].lowestCost <=
                        moveQuantity;
            tColor = unit.getPlayer() == getCurrentPlayer() ? moveColor
                                                            : targetColor;
            ...
            HealerUnit healer = controller.getMode() == GameController.Mode.HEAL
                                    ? (HealerUnit)unit
                                    : null;
            TransportUnit transporter =
                controller.getMode() == GameController.Mode.UNLOAD_LOCATE
                    ? (TransportUnit)unit
                    : null;
            ...
            for (Direction d : Direction.cardinalDirections()) {
                ...
                if ((controller.getMode() == GameController.Mode.HEAL &&
                     healer.canHeal(getUnit(xx, yy))) ||
                    (controller.getMode() == GameController.Mode.LOAD &&
                     getUnit(xx, yy) instanceof TransportUnit &&
                     ((TransportUnit)getUnit(xx, yy)).canCharge(unit)) ||
                    (controller.getMode() ==
                         GameController.Mode.UNLOAD_LOCATE &&
                     transporter.canRemove(controller.getTransportUnit(), xx,
                                           yy)))
                    targets[yy][xx] = true;
            }
            upperLeft.move(Math.max(0, unit.getX() - 1),
                           Math.max(0, unit.getY() - 1));
            lowerRight.move(Math.min(getMapWidth(), unit.getX() + 2),
                            Math.min(getMapHeight(), unit.getY() + 2));
            tColor = controller.getMode() == GameController.Mode.HEAL
                         ? healColor
                         : loadColor;
    ...
    public boolean updateTarget(Point p) {
        ...
        if (controller.getMode() == GameController.Mode.MISSILE_LAUNCHER) {
            ...
            lowerRight.move(Math.min(getMapWidth(), p.x + 4),
                            Math.min(getMapHeight(), p.y + 4));
            tColor = targetColor;
            return true;
        }
        return false;
    ...
    public void flash(String message, int x, int y, int time) {
        flashs.add(
            new FlashMessage(message, x, y, time, FlashMessage.Type.SUCCESS));
    }
    ...
    static {
        ...
        explosionBis =
            new Image[] {d.getImage(0, 33, 30, 30), d.getImage(0, 67, 30, 30),
     ...
     * @param naval is true to display the sink animation and false for the
     *     explosion
     */
    ...
    class DeathAnimation {
        ...
        public Image getImage() { return animation[(time - 1) / 5]; }
    }
    ...
    class MissileAnimation {
        ...
        public MissileAnimation(MissileLauncher missile, int endX, int endY) {
            ...
            this.beginX =
                (missile.getX() - controller.camera.getX()) * MainFrame.UNIT;
            this.beginY =
                (missile.getY() - controller.camera.getY()) * MainFrame.UNIT;
            this.x = beginX;
            this.y = beginY - (controller.camera.getY() - 1) * MainFrame.UNIT -
                     UniverseRenderer.launch.getHeight(controller.makeView());
            ...
            for (int i = -3; i < 4; i++)
                for (int j = -3; j < 4; j++) {
                    ...
                    if (Math.abs(i) + Math.abs(j) <= 3 && unit != null &&
                        !damages.containsKey(unit))
        ...
        public void draw(Graphics g) {
            if (!goingUp && x == endX && y >= endY) { // end of the animation
                ...
                if (explosionNumber < 24) { // display final explosion
                    for (int i = -3; i < 4; i++)
                        for (int j = -3; j < 4; j++) {
                            int tmpX = endX + i * MainFrame.UNIT,
                                tmpY = endY + j * MainFrame.UNIT;
                            if (Math.abs(i) + Math.abs(j) <= 3 && tmpX >= 0 &&
                                tmpY >= 0 &&
                                tmpX < (controller.camera.getX() +
                                        controller.camera.width) *
                                           MainFrame.UNIT &&
                                tmpY < (controller.camera.getY() +
                                        controller.camera.height) *
                                           MainFrame.UNIT)
                                g.drawImage(explosionBis[explosionNumber / 8],
                                            tmpX, tmpY, controller.makeView());
                        }

                    explosionNumber++;
                } else { // do the actual damages and delete this animation
                    missile.fire(
                        endX / MainFrame.UNIT + controller.camera.getX(),
                        endY / MainFrame.UNIT + controller.camera.getY());

                    for (Map.Entry<AbstractUnit, Integer> e :
                         damages.entrySet())
                        flash("" + (e.getKey().getLife() - e.getValue()),
                              (e.getKey().getX() - controller.camera.getX() +
                               1) * MainFrame.UNIT +
                                  5,
                              (e.getKey().getY() - controller.camera.getY()) *
                                      MainFrame.UNIT +
                                  5,
                              1000, FlashMessage.Type.ALERT);
                    UniverseRenderer.this.missileAnimation = null;
                    controller.setMode(GameController.Mode.MOVE);
                }
            } else { // display the rise and fall of the missile
                if (y <= -fly.getHeight(controller.makeView()) -
                             smokeUp.getHeight(controller.makeView())) {
                    y = 0;
                    goingUp = false;
                    x = endX;
                }
                ...
                if (goingUp) { // rise of the missile
                    if (beginY + MainFrame.UNIT - y <=
                        fly.getHeight(controller.makeView()))
                        g.drawImage(
                            launch,
                            x + (MainFrame.UNIT -
                                 launch.getWidth(controller.makeView())) /
                                    2,
                            y, controller.makeView());
                    else if (beginY + MainFrame.UNIT - y >=
                             fly.getHeight(controller.makeView()) +
                                 smokeUp.getHeight(controller.makeView())) {
                        // engines are turned on
                        g.drawImage(fly,
                                    x + (MainFrame.UNIT -
                                         fly.getWidth(controller.makeView())) /
                                            2,
                                    y, controller.makeView());
                        g.drawImage(
                            smokeUp,
                            x + (MainFrame.UNIT -
                                 smokeUp.getWidth(controller.makeView())) /
                                    2,
                            y + fly.getHeight(controller.makeView()),
                            controller.makeView());
                    } else
                        g.drawImage(fly,
                                    x + (MainFrame.UNIT -
                    ...
                    g.drawImage(arrival,
                                x + (MainFrame.UNIT -
                                     arrival.getWidth(controller.makeView())) /
                                        2,
                                y - arrival.getHeight(controller.makeView()),
                                controller.makeView());
                    g.drawImage(
                        smokeDown,
                        x + (MainFrame.UNIT -
                             smokeDown.getWidth(controller.makeView())) /
                                2,
                        y - arrival.getHeight(controller.makeView()) -
                            smokeDown.getHeight(controller.makeView()),
                        controller.makeView());
                }
