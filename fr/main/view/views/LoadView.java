...
public class LoadView extends View {
    ...
    private static int width = 350,
                       height = 50, x = (MainFrame.width() - width) / 2,
    ...
    private static class AdvicePanel extends JLabel {
        ...
        public AdvicePanel(Random rand) {
            super("<html><em style='text-align:center'>" +
                  sentences[rand.nextInt(sentences.length)] + "</em></html>");
            setFont(font);
    ...
    public void paintComponent(Graphics g) {
        ...
        g.setFont(font);
        g.drawImage(bkg, 0, 0, MainFrame.width(), MainFrame.height(),
                    Color.black, null);

        g.setColor(Color.black);
        g.drawRect(x, y, width, height);
        g.setColor(Color.green);
        g.fillRect(x + 1, y + 1, controller.getLoad() * width / 100,
                   height - 1);
    }
}
