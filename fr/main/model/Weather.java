 ...
 */
public enum Weather implements java
.io.Serializable {

    SUNNY("Ensoleillé", false, 1, 0), FOGGY("Brumeux", true, 1, 0),
        RAINY("Pluvieux", true, 1, 1), SNOWY("Neigeux", true, 2, 2);

    /**
     * malusFuel : the multiplying factor of the quantity of fuel consumed
     * malusVision : the amount removed to the vision of the units when there is
     * this weather
     */
    ...
    /**
     * @param fog is true if and only if the basic weather (SUNNY or FOGGY)
     *     returned has fog (so FOGGY if true and SUNNY otherwise)
     * @return a random Weather with 5% chance of being SNOWY, 10% chance of
     *     being RAINY, and 85% chance of being the other one
     */
    public static Weather random(boolean fog) {
        if (fog)
            return random(0, 85, 10, 5);
        else
            return random(85, 0, 10, 5);
    }
