...
public class BuildingRenderer {
    ...
    protected static HashMap<AbstractBuilding, BuildingRender> renderers =
        new HashMap<>();
    ...
    public static BuildingRender getRender(AbstractBuilding building) {
        if (renderers.containsKey(building))
            return renderers.get(building);

        // TODO: make dynamic
        if (building instanceof Dock)
            renderers.put(building, new DockRenderer(building));
        else if (building instanceof Airport)
            renderers.put(building, new AirportRenderer(building));
        else if (building instanceof Barrack)
            renderers.put(building, new BarrackRenderer(building));
        else if (building instanceof City)
            renderers.put(building, new CityRenderer(building));
        else if (building instanceof Headquarter)
            renderers.put(building, new HeadquarterRenderer(building));
        else if (building instanceof MissileLauncher)
            renderers.put(building, new MissileLauncherRenderer(building));
        return renderers.get(building);
    }

    public static void render(Graphics g, Point coords,
