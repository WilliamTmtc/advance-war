...
public class UnitRenderer {
    ...
    protected static final HashMap<Class<? extends AbstractUnit>,
                                   Function<AbstractUnit, ? extends Render>>
        constructors;

    static {
        constructors = new HashMap<Class<? extends AbstractUnit>,
                                   Function<AbstractUnit, ? extends Render>>();
    ...
    public static abstract class Render extends Renderer {
        ...
        public final void deathSound() { death.playOneTime(); }

        public final void selectedSound() { selected.playOneTime(); }

        public final void attackSound() { attack.playOneTime(); }

        public boolean moveOffset(Direction d) { return moveOffset(d, true); }

        public void cancelOffset() { offset.move(0, 0); }

        public boolean moveOffset(Direction d, boolean change) {
            d.move(offset);
            if (change && (Math.abs(offset.x) >= MainFrame.UNIT ||
                           Math.abs(offset.y) >= MainFrame.UNIT)) {
                cancelOffset();
                return unit.move(d);
            }
            return true;
        ...
        public void draw(Graphics g, int x, int y) {
            ...
            if (unit.getLife() < 34)
                g.drawImage(TerrainPanel.lifeImage, x + offset.x, y + offset.y,
                            null);

            // draw a jerrycan to show that the unit's fuel is low
            if (unit.getFuel().getQuantity() <=
                unit.getFuel().maximumQuantity / 2)
                g.drawImage(TerrainPanel.fuelImage, x + offset.x,
                            y + offset.y + MainFrame.UNIT / 2, null);
            ...
            if (pr != null && pr.getAmmunition() <= pr.maximumAmmo / 2)
                g.drawImage(TerrainPanel.ammoImage,
                            x + offset.x + MainFrame.UNIT / 2, y + offset.y,
                            null);

            // draw a lock to show that the unit is hidden
            if (unit instanceof HideableUnit && ((HideableUnit)unit).hidden())
                g.drawImage(lockImage, x + offset.x + MainFrame.UNIT / 2,
                            y + offset.y + MainFrame.UNIT / 2, null);
        }
    ...
    public static Render getRender(AbstractUnit unit) {
        ...
        for (Map.Entry<Class<? extends AbstractUnit>,
                       Function<AbstractUnit, ? extends Render>> entry :
    ...
    public static void remove(AbstractUnit u) { remove(renderers.get(u)); }
